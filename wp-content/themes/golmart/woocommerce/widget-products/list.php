...
<?php while ( $loop->have_posts() ) : $loop->the_post(); global $product; ?>
	<?php
        ...
        if ( 0 == $_count % $columns_count )
            $class_fix .= ' last';
    ?>
    <?php if( $_count%$columns_count == 1 ) echo '<div class="clearfix rows-products product_list_widget no-padding">'; ?>
	<div class="<?php echo esc_attr($class_column.$class_fix); ?> wow fadeInUp product-wrapper" data-wow-duration="0.6s" data-wow-delay="<?php echo esc_attr( $_delay ); ?>ms">
		<?php wc_get_template( 'content-widget-product.php', array( 'show_rating' => true , 'animate'=> true , 'delay'=>$_delay ) ); ?>
	</div>
