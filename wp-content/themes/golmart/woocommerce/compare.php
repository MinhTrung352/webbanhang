...
<table class="table table-bordered compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php else : ?>
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr($product_class); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ?>" data-product_id="<?php echo esc_html($product->id); ?>"><?php _e( 'Remove', 'yit' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
        ...
        <?php foreach ( $fields as $field => $name ) : ?>
            ...
            <tr class="<?php echo esc_html($field) ?>">
        ...
        <?php if ( $repeat_price == 'yes' && isset( $fields['price'] ) ) : ?>
            <tr class="price repeated">
                <th><?php echo esc_attr( $fields['price']); ?></th>

                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                    <td class="<?php echo esc_attr($product_class); ?>"><?php echo esc_attr( $product->fields['price'] ) ?></td>
                <?php endforeach; ?>
        ...
        <?php if ( $repeat_add_to_cart == 'yes' && isset( $fields['add-to-cart'] ) ) : ?>
            <tr class="add-to-cart repeated">
                <th><?php echo esc_attr( $fields['add-to-cart'] ) ?></th>
