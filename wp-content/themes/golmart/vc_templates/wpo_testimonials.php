	...
	<?php if($query->have_posts()){ ?>
		<?php if($title!=''){ ?>
			<h3 class="widget-title visual-title <?php echo esc_attr( $size ).' '.esc_attr( $alignment ); ?>">
				<span><span><?php echo esc_html( $title ); ?></span></span>
			</h3>
		<?php } ?>
		<?php if($skin=='skin-1'){ ?>
			<!-- Skin 1 -->
		...
		<?php }else if($skin == 'skin-2'){ ?>
			...
			<div id="carousel-<?php echo esc_attr( $_id ); ?>" class="widget-content carousel slide" data-ride="carousel">
				...
				<div class="carousel-inner testimonial-carousel">
					<?php while($query->have_posts()):$query->the_post(); ?>
						<!-- Wrapper for slides -->
						<?php if( $_count%2 == 0 ) echo '<div class="item'.(($_count==0)?" active":" ").'">'; ?>
							<div class="col-md-6">
								<div class="testimonial-description">
									<?php the_content() ?>
								</div>
						...
						<?php if( ($_count%2==1 && $_count!=0) || $_count== $query->found_posts ) echo '</div>'; ?>
						<?php $_count++; ?>
					<?php endwhile; ?>
					<?php wp_reset_postdata(); ?>
				</div>
