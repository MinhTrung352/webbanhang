...
<section class="widget <?php echo esc_attr( $layout ); ?> wpo-grid-posts section-blog <?php echo (($el_class!='')?' '.esc_attr( $el_class ):''); ?>">
    <?php if( $title ) { ?>
    ...
    <div class="widget-content">
        ...
        <?php
            ...
            if($loop->have_posts()){  ?>
                <?php wpo_get_template('post/'.$layout.'.php',array( 'grid_columns' => $grid_columns, 'loop'=> $loop , 'class_column'=> $el_class,'grid_thumb_size'=>$grid_thumb_size,'layout' => $layout ) ); ?>
             <?php  } ?>
          <?php  } ?>
          <?php wp_reset_postdata(); ?>
    </div>
