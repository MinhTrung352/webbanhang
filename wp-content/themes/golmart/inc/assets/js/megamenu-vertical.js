...
(function($) {
	$.fn.PavMegamenuEditor = function(opts) {
		...
		function processMenu( item , _parent, _megamenu ){
			...
			$(".form-setting").hide();
		    $("#menu-form").show();
		    var $screenshot = $('#menu-form .screenshot');
			$.each( $("#menu-form form").serializeArray(), function(i, input ){
				...
				if(input.name=='menu_image'){
					if(val!='')
						$screenshot.html('<img src="' + val + '">').show();
					else
		...
		function menuForm(){
			...
			$("#menu-form .text_customize").bind({
				paste : function(){
					setTimeout( function() { setTextCustomize(); }, 100);
				},
				keypress : function(){
					setTimeout( function() { setTextCustomize(); }, 100);
				}
		...
		function setImage(){
			$(activeMenu).data('image', $("#menu-form .menu_image").val() );
		}
		...
		function setTextCustomize(){
			$(activeMenu).data('text_customize', $("#menu-form .text_customize").val() );
		}
 			  ...
 			  */
 			  $("#unset-data-menu").click( function(){
 				 if( confirm('Are you sure to reset megamenu configuration') ){
 				    $.ajax({
						url: config.action,
						data: 'reset_vertical=1&menugroup='+$("#menugroup").val(),
						type:'POST',
						}).done(function( data ) {
					 		 location.reload();
				    });
		...
		function recalculateColsWidth(){
	 	...
	 	function saveMenuData(){
	 	 	var output = new Array();
 	 	 	...
 	 	 	var params = 'menugroup='+$("#menugroup").val()+'&params_vertical='+j;
 	 	 	$.ajax({
