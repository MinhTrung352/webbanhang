jQuery(document).ready(function($){
	...
	function optionsframework_add_file(event, selector) {
		...
		if ( optionsframework_upload ) {
			...
			optionsframework_upload.on( 'select', function() {
				...
				optionsframework_selector.find('.menu_image').val(attachment.attributes.url).change();
	...
	$('body').delegate('.remove-image', 'click', function(event) {
		optionsframework_remove_file( $(this).parent().parent() );
	});

	$('body').delegate('.menu_image_action .add-image', 'click', function(event) {
