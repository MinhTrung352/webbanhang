   ...
   public  function start_lvl(  &$output, $depth = 0, $args = array(), $megaconfig=null ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if($depth==0){
			...
			if( isset($item->megaconfig->image) &&  $item->megaconfig->image ){
				$item_output .= "<img class='icon-ver' src='".$item->megaconfig->image."' />";
			}elseif(isset($item->megaconfig->icon) && $item->megaconfig->icon !=''){
	...
	protected function renderAttrs( $menu, $args ,$level){
		if( $this->isLiveEdit ) {
			...
			$t .= ' data-image="'.(isset($menu->megaconfig->image)?$menu->megaconfig->image:"").'"';
			return $t;
		}else{
			$t = sprintf( ' data-id="%s"   ', $menu->ID );
