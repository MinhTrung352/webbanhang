...
if(!class_exists('WPO_MegamenuEditor')){
		...
		class WPO_MegamenuEditor{
			...
			public function adminLoadMenu(){ 
				...
				add_theme_page( 'Megamenu', $this->l("Megamenu Editor"), 'administrator', 'wpo_megamenu', array($this,'megaMenuPage') );
				add_theme_page( 'Megamenu', $this->l("Megamenu Vertical Editor"), 'administrator', 'wpo_megamenu_vertical', array($this,'megaMenuVerticalPage') );
			}

			public function megaMenuVerticalPage(){
	  			$wpoEngines = WPO_Shortcodes::getInstance();
	  			$widgets = WPO_Megamenu_Widget::getInstance()->getWidgets();
				require( WPO_FRAMEWORK_PATH. 'megamenu/templates/editor-vertical.php' );
			}


			public function megaMenuPage(){
	  			$wpoEngines = WPO_Shortcodes::getInstance();
	  			$widget = WPO_Megamenu_Widget::getInstance()->loadWidgets();

	  			$widgets = $widget->getWidgets();

				require( WPO_FRAMEWORK_PATH . 'megamenu/templates/editor.php' );
			}
			...
			public function initScripts(){
				...
				if( isset($_GET['page']) && ( $_GET['page']=='wpo_megamenu' || $_GET['page']=='wpo_megamenu_vertical' ) ){
					...
					wp_enqueue_style('megamenu_bootstrap_css',WPO_THEME_URI.'/css/bootstrap.css');
					...
					wp_enqueue_style( 'wpo_shortcode_css',WPO_FRAMEWORK_ADMIN_STYLE_URI.'css/shortcode.css');
					if($_GET['page']=='wpo_megamenu'){
						wp_enqueue_script('wpo_megamenu_js', WPO_FRAMEWORK_ADMIN_STYLE_URI.'js/megamenu.js',array(),false,true);
						wp_enqueue_style( 'wpo_megamenu_css', WPO_FRAMEWORK_ADMIN_STYLE_URI.'css/megamenu.css');
					}else if($_GET['page']=='wpo_megamenu_vertical'){
						wp_enqueue_script('wpo_megamenu_js',WPO_FRAMEWORK_ADMIN_STYLE_URI.'js/megamenu-vertical.js',array(),false,true);
						wp_enqueue_style( 'wpo_megamenu_css',WPO_FRAMEWORK_ADMIN_STYLE_URI.'css/megamenu-vertical.css');
						wp_enqueue_script('wpo_upload_js',WPO_FRAMEWORK_ADMIN_STYLE_URI.'js/upload.js',array(),false,true);
					}

					wp_enqueue_script('wpo_shortcode_js',WPO_FRAMEWORK_ADMIN_STYLE_URI.'js/shortcode.js',array(),false,true);
			...
			public function register_megamenu_setting() {  
				if( isset($_GET['page']) && $_GET['page'] == 'wpo_megamenu'&& isset($_POST) ){
					...
					$menugroup = wpo_theme_options('magemenu-menu','');
					if( isset($_POST['menugroup']) && $_POST['menugroup'] ){
						$menugroup = (int)$_POST['menugroup'];
					}
					...
					if( isset($_POST['params']) && !empty($_POST['params']) ) {
						...
						if( $params ){
							...
							$a = serialize( $output );
							update_option( "WPO_MEGAMENU_DATA-".$menugroup, $a );
						}
						exit();
					}
					if( isset($_POST['reset']) && $_POST['reset'] ){
						update_option( "WPO_MEGAMENU_DATA-".$menugroup, null );
						exit();
					}

					if( isset($_POST['params_vertical']) && !empty($_POST['params_vertical']) ) {
						$params =  $_POST['params_vertical'];
				$params = stripslashes(trim(html_entity_decode( $params ))) ;
						if( $params ){
							$a = json_decode(($params));
							$output = array();
							foreach( $a as $d ){
							...
							$a = serialize( $output );
							update_option( "WPO_MEGAMENU_VERTICAL_DATA-".$menugroup, $a );
						}
						exit();
					}
					if( isset($_POST['reset_vertical']) && $_POST['reset_vertical'] ){
						update_option( "WPO_MEGAMENU_VERTICAL_DATA-".$menugroup, null );
						exit();
					}
				}
			}
			...
			public function ajax_shortcode_save(){
				...
				if($id==0){
					...
					$response['title']=$name;
			$response['message'] = __('Widgets published', TEXTDOMAIN);
					$response['type_widget'] = $type;
				}else{
					$response['type']='update';
			$response['message'] = __('Widgets updated', TEXTDOMAIN);
					$response['title']=$name;
					$response['id']=$id;
			...
			static function megamenu_uploader() {
				?>
					<div class="screenshot"></div>
					<input type="hidden" class="menu_image" name="menu_image">
					<div class="menu_image_action">
