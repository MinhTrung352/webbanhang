...
if(function_exists('PostRatings')){
  ...
  ?>
      <div class="entry-vote <?php echo trim( $classRating ); ?>"><span class="entry-vote-inner"><?php echo  $result_ratings ?></span></div>
  <?php
    }
  }
}
...
if(!function_exists('wpo_categories_searchform')){
    function wpo_categories_searchform(){
		...
		<form role="search" method="get" class="input-group search-category" action="<?php echo esc_url( home_url('/') ); ?>">
            <input name="s" id="s" maxlength="60" class="form-control search-category-input" type="text" size="20" placeholder="<?php echo __('What do you need...', TEXTDOMAIN); ?>"> 
...
if(!function_exists('wpo_renderButtonToggle')){
    ...
    ?>
      <a href="javascript:;"
            ...
            data-toggle="<?php echo trim($toggle); ?>"
...
function wpo_get_widget_block_styles(){
