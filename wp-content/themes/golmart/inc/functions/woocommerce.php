...
add_filter('add_to_cart_fragments',             'wpo_woocommerce_header_add_to_cart_fragment_mobile' );
...
function wpo_woocommerce_breadcrumbs() {
	return array(
        ...
        'wrap_before' => '<section  class="wrapper-breadcrumb"><section id="breadcrumb" class="breadcrumb wpo-breadcrumb"><nav class="container" itemprop="breadcrumb">',
        'wrap_after'  => '</nav></section></section>',
        'before'      => '',
        'after'       => '',
...
function wpo_woocommerce_header_add_to_cart_fragment_mobile( $fragments ) {
    global $woocommerce;
    ob_start();
    wpo_cartdropdown_mobile();
    $fragments['#cart_moblie'] = ob_get_clean();
    return $fragments;
}
    ...
    if(WPO_WOOCOMMERCE_ACTIVED){
        ...
        ?>
            <div id="cart" class="dropdown clearfix">
                ...
                <a class="dropdown-toggle mini-cart" data-toggle="dropdown" aria-expanded="true" role="button" aria-haspopup="true" data-delay="0" href="#" title="<?php _e('View your shopping cart', TEXTDOMAIN); ?>">
                    <span class="title-cart"><?php _e('shopping cart',TEXTDOMAIN); ?></span>
                    <?php echo sprintf(_n(' <span class="mini-cart-items"> %d '.__('item', TEXTDOMAIN).' - </span> ', ' <span class="mini-cart-item"> %d '.__('items', TEXTDOMAIN).' - </span> ', $woocommerce->cart->cart_contents_count, TEXTDOMAIN), $woocommerce->cart->cart_contents_count);?> <?php echo ( $woocommerce->cart->get_cart_total() ); ?>
                </a>
...
if(!function_exists('wpo_cartdropdown_mobile')){
 function wpo_cartdropdown_mobile(){
    if(WPO_WOOCOMMERCE_ACTIVED){
        global $woocommerce;
        ?>   
            <div id="cart_moblie" class="cart_mobile">
                    <a class="dropdown-toggle mini-cart" data-toggle="dropdown" aria-expanded="true" role="button" aria-haspopup="true" data-delay="0" href="#" title="<?php _e('View your shopping cart', TEXTDOMAIN); ?>">
                        ...
                        <?php echo sprintf(_n(' <span class="mini-cart-items"> %d '.__('items', TEXTDOMAIN).' - </span> ', ' <span class="mini-cart-items"> %d '.__('item', TEXTDOMAIN).' - </span> ', $woocommerce->cart->cart_contents_count, TEXTDOMAIN), $woocommerce->cart->cart_contents_count);?> <?php echo trim( $woocommerce->cart->get_cart_total() ); ?>
                    </a>
            </div>     
        <?php
        }
...
if( wpo_theme_options('wc_cartnotice') ) {
    ...
    function WPO_jsWoocommerce(){
        ...
        wp_register_script( 'noty_js', WPO_THEME_URI.'/js/jquery.noty.packaged.js', array( 'jquery' ) );
...
function wpo_custom_onsale_text( $text, $post, $product ) {
     ...
     $sale = '<span class="onsale"><span class="label-sale">' . __( 'Sale', TEXTDOMAIN) . '</span></span>';
...
add_action( 'wp_ajax_wooloadproducts', 'wpo_ajax_load_products' );
add_action( 'wp_ajax_nopriv_wooloadproducts', 'wpo_ajax_load_products' );

function wpo_ajax_load_products(){
    ...
    $loop = wpo_woocommerce_query( $args['type'], $args['limit'] ,  $args['slug'] );

    $number = $args['show'];
    $_id = wpo_makeid();
    ?>
    <div class="grid-wrapper">
        <div id="carousel-<?php echo esc_attr($_id); ?>" class="filled-content text-center owl-carousel-play" data-ride="owlcarousel">
            <div class="owl-carousel rows-products" data-slide="<?php echo esc_attr($number); ?>" data-pagination="false" data-navigation="true">
