...
class WPO_Frontend extends WPO_Framework {
	...
	public function __construct(){
		...
		if(WPO_POSTTYPE_ACTIVED){
			$this->addPostTypeSuport( array( 'brands','testimonials','portfolio', 'footer' ) );
		}
	...
	public function loadFrontEndMedia(){
		$this->addScript('owlcaousel-js',WPO_THEME_URI.'/js/owl-carousel/owl.carousel.min.js',array(),true,true);	
	...
	//	$this->addStyle('owlcaousel-css',WPO_THEME_URI.'/js/owl-carousel/owl.carousel.css' );
	...
	public function getPageConfig(){
		...
		$pageconfig = wp_parse_args((array) $config, $defaults);
		if( empty($config ) ){
			$pageconfig['breadcrumb'] = true; 
		}
		$config = array();
		if($pageconfig==''){
			$lt = '0-1-0';
			...
			$config['footer_style']= $pageconfig['footer_style'];
	...
	private function initRequirePlugin(){
		...
		$this->addRequiredPlugin(array(
			'name'                     	=> 'WPO Posttypes', // The plugin name
		    'slug'                     	=> 'wpo_posttypes', // The plugin slug (typically the folder name)
		    'source'				   => 'http://www.wpopal.com/thememods/wpo_posttypes.zip' ,// If false, the plugin is only 'recommended' instead of required
		    'required' 					=> true
		));
	...
	*/
	private function setSidebarDefault(){
		$this->addSidebar('sidebar-default',
			array(
				'name'          => __( 'Sidebar Default', TEXTDOMAIN ),
				'id'            => 'sidebar-default',
				'description'   => __( 'Appears on posts and pages in the sidebar.', TEXTDOMAIN),
		...
		$this->addSidebar('twitter',
			array(
				...
				'id'            => 'twitter',
				'description'   => __( 'Appears on content in the sidebar.', TEXTDOMAIN),
		...
		$this->addSidebar('vertical-menu',
			array(
				'name'          => __( 'Vertical Menu' , TEXTDOMAIN),
				'id'            => 'vertical-menu',
				'description'   => __( 'Appears on posts and pages in the sidebar.', TEXTDOMAIN),
				'before_widget' => '<aside id="%1$s" class="widget  clearfix %2$s">',
				'after_widget'  => '</aside>',
		...
		$this->addSidebar('header-sport-right',
			array(
				'name'          => __( 'Header Sport Right' , TEXTDOMAIN),
		...
		$this->addSidebar('header-defaul-center',
			array(
				'name'          => __( 'Header Default Center' , TEXTDOMAIN),
				'id'            => 'header-defaul-center',
				'description'   => __( 'Appears on posts and pages in the header.', TEXTDOMAIN),
				'before_widget' => '<aside id="%1$s" class="widget no-margin clearfix %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h3 class="widget-title"><span>',
				'after_title'   => '</span></h3>',
		...
		$this->addSidebar('header-mobile-right',
			array(
				'name'          => __( 'Header Mobile Right' , TEXTDOMAIN),
		...
		$this->addSidebar('header-market-center',
			array(
				'name'          => __( 'Header Market Center' , TEXTDOMAIN),
				'id'            => 'header-market-center',
				'description'   => __( 'Appears on posts and pages in the Header.', TEXTDOMAIN),
				'before_widget' => '<aside id="%1$s" class="widget no-margin clearfix %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h3 class="widget-title"><span>',
		...
		$this->addSidebar('header-market-bottom',
			array(
				'name'          => __( 'Header Market bottom' , TEXTDOMAIN),
				'id'            => 'header-market-bottom',
				'description'   => __( 'Appears on posts and pages in the Header.', TEXTDOMAIN),
				'before_widget' => '<aside id="%1$s" class="widget no-margin clearfix %2$s">',
				'after_widget'  => '</aside>',
				'before_title'  => '<h3 class="widget-title"><span>',
		...
		$this->addSidebar('sidebar-right',
			...
			array(
				'name'          => __( 'Blog Left Sidebar' , TEXTDOMAIN),
			...
			array(
				'name'          => __( 'Blog Right Sidebar', TEXTDOMAIN),
	...
	public function getHeaderLayout( $page_id = null){
		global $wp_query;

		if( !empty($page_id) ){
			$layout = get_post_meta( $page_id, 'wpo_pageconfig', true );
		}else
		...
		elseif( !isset($layout['header_skin']) || isset( $layout['header_skin'] ) && $layout['header_skin'] =='default' )
			return '';
		else
			return $layout['header_skin'];
	}

	public function wpo_getFooterStyle( $footer_style ){

	    if( !isset( $footer_style['footer_style']) || isset( $footer_style['footer_style'] ) && $footer_style['footer_style'] =='global' )
			return wpo_theme_options('footer-style','default');
		else
			return $footer_style['footer_style'];
	}
}
