...
$terms = get_terms('category_portolio',array('orderby'=>'id'));
...
<?php if( $portfolio->have_posts()): ?>
    ...
    <?php if ( count($terms) > 0 ){
          foreach ( $terms as $term ): ?>
            <li><a href="javascript:void(0)"  data-filter=".<?php echo esc_attr($term->slug); ?>">
  ...
  <?php while($portfolio->have_posts()): $portfolio->the_post();
     $item_classes = 'all ';
     $item_cats = get_the_terms( $portfolio->post->ID, 'category_portolio' );
...
<?php }elseif ( has_post_thumbnail() ){ ?>
      ...
      <?php
      if( $image_attributes ) : ?>
      <a href="<?php echo esc_url_raw( $image_attributes[0] ); ?>" rel="prettyPhoto[all]" title="<?php the_title_attribute(); ?>" class="btn btn-outline-inverse">
